...
/*		double x0 = s.getP0().getX();
	...
	public boolean overlaps(Rectangle other) {
		return contains(other) || belongsTo(other)
				|| partlyOverlaps(other);
	}
	
	public boolean partlyOverlaps(Rectangle other) {
		/* this is when at least one of 1's ribs intersects with 2's rib
		   there is an even count of such intersections 
		   but we will look for just first one */
		for (Segment s : this.segments())
			for (Segment q : other.segments())
				if (s.intersectsSegment(q)) return true;
		return false;
	}
